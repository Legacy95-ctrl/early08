<?php
// RCC asset endpoint: shirt xml pointing at its texture (ported from zyphie asset/shirt.php)
require __DIR__ . '/../includes/config.php';

$id = (int)($_REQUEST['id'] ?? 0);
$stmt = db()->prepare('SELECT * FROM catalog_items WHERE id = ? AND asset_type_id = 3');
$stmt->execute([$id]);
$asset = $stmt->fetch();
if (!$asset) {
...
<Properties>
  <Content name="ShirtTemplate"><url>http://' . $sitelinkconnection . '/Thumbs/Item.php?id=' . (int)$asset['id'] . '&amp;real=1</url></Content>
