<?php
// RCC asset endpoint: t-shirt graphic xml (ported from zyphie asset/tshirt.php)
require __DIR__ . '/../includes/config.php';

$id = (int)($_REQUEST['id'] ?? 0);
$stmt = db()->prepare('SELECT * FROM catalog_items WHERE id = ? AND asset_type_id = 5');
$stmt->execute([$id]);
$asset = $stmt->fetch();
if (!$asset) {
...
<Properties>
  <Content name="Graphic"><url>http://' . $sitelinkconnection . '/Thumbs/Item.php?id=' . (int)$asset['id'] . '&amp;real=1</url></Content>
